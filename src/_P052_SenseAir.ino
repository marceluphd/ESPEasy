...
String Plugin_052_log_buffer(byte* buffer, int length) {
  String log = F("Modbus buffer: ");
  log.reserve(log.length() + 3*length + 5);
...
bool Plugin_052_validChecksum() {
  if (_plugin_052_recv_buf_used > 4) {
    byte checksumHi = 0;
    byte checksumLo = 0;
    Plugin_052_ModRTU_CRC(_plugin_052_recv_buf, (_plugin_052_recv_buf_used - 2), checksumHi, checksumLo);
    if (_plugin_052_recv_buf[_plugin_052_recv_buf_used - 2] == checksumLo &&
        _plugin_052_recv_buf[_plugin_052_recv_buf_used - 1] == checksumHi) {
      return true;
    }
  }
...
bool Plugin_052_processModbusException() {
  const byte received_functionCode = _plugin_052_recv_buf[1];
  const byte value = _plugin_052_recv_buf[2];
  ...
  switch (value) {
    ...
    default:
      log += String(F("Unknown Exception. function: "))+ received_functionCode;
      log += String(F(" Exception code: ")) + value;
      break;
  }
  log += F(" - ");
  log += Plugin_052_log_buffer(_plugin_052_recv_buf, _plugin_052_recv_buf_used);
...
int Plugin_052_processCommand()
{
  ...
  byte checksumHi = 0;
  byte checksumLo = 0;
  Plugin_052_ModRTU_CRC(_plugin_052_sendframe, _plugin_052_sendframe_used, checksumHi, checksumLo);
  ...
  if (!Plugin_052_validChecksum() || !Plugin_052_processModbusException()) {
    String log = F("Invalid reponse, sent frame: ");
...
void Plugin_052_modbus_log_MEI(byte slaveAddress) {
  for (int device_id = 1; device_id <= 4; ++device_id) {
    ...
    while (more_follows) {
      ...
      if (!more_follows && object_id < object_id_hi) {
        more_follows = true;
        object_id++;
      }
    }
